...
struct Joint {
    ...
    Vec3 kp{0,0,0};              // per-joint PD stiffness (GaitNet); Revolute uses [0]
    Vec3 kv{0,0,0};              // per-joint PD damping (GaitNet)
...
struct EnvConfig {
    ...
    double defaultKp = 200.0, defaultKv = 40.0, damping = 0.1;
    ...
    bool bvh_symmetry = true, bvh_heightCalibration = true, cyclicbvh = true;
    bool residual = true; int simHz = 480, controlHz = 30, inferencePerSim = 1;
    ...
    bool enforceSymmetry = true; double actionScale = 0.04, timeWarping = 0.001;
    bool stanceLearning = false, metabolicReward = true;
    ...
    std::string eoeType = "tuple", rewardType = "gaitnet";
    double headLinearAccWeight = 4.0, headRotWeight = 4.0, stepWeight = 2.0;
    double metabolicWeight = 8.0, avgVelWeight = 6.0;
};
struct ParamRange {
    std::string name; double min = 1.0, max = 1.0, def = 1.0;
    std::string sampling; bool hasDefault = false, hasSampling = false;
};
struct Parameters {
    std::vector<ParamRange> gait, skeleton, torsion, muscle_length, muscle_force;
};
